...
Modules:
  ...
  roles.py              coach_id -> role map (SOURCE OF TRUTH for which coaches appear)
...
Changes:
  10-Jul-2026 IST  _apply_roles(): attaches `role` from roles.py and KEEPS ONLY the coaches
                   listed there (roles.py is the source of truth; unmapped coaches are dropped
                   from every view, not bucketed as "Other"). Applied in both run_data() and
                   main() before calculate(). Prints "kept N / dropped M" each run.
  10-Jul-2026 IST  run_data(): stage-level timing (fetch_availability / apply_roles /
                   fetch_consumed / calculate / build_data / TOTAL), printed with flush=True so
                   the lines show up immediately in Vercel logs. Measured cold: fetch = 10.5s of
                   an 11.1s total, calculate = 0.62s -> refresh cost is network, not compute.
  10-Jul-2026 IST  Both paths pass now=tzutil.now_ist() into calculate() (elapsed-slot trim).
                   Added --now "YYYY-MM-DD HH:MM" to simulate a refresh time (defaults to real
                   IST now). Both paths unpack the new 5th return value `blk7` and forward it to
                   write_excel() and build_data_from_engine().

Known issue (open):
  run_data() fetches consumed for the ANALYSIS WINDOW only (the month), but the open-slot finder
  spans today..today+6. In the last 6 days of any month the next month's slots have no consumed
  data, so booked/blocked are not subtracted and open counts are inflated. It self-corrects at
  each month rollover and recurs the following month.
  Fix: consumed_end = max(w1, today + pd.Timedelta(days=6)) before fetch_consumed(), in both
  run_data() and main().
"""
...
def run_data(start, end, today=None, status=DEFAULT_STATUSES, exclude_time_slot="", exclude_coach=""):
    ...
    # consumed window auto-follows the 7-day finder window so month-boundary spillover
    # (e.g. window ends 31-Jul but finder runs into Aug) still has consumed data to subtract
    ...
    g, inst, open7, (n0, n1), blk7 = ca.calculate(slots_df, consumed_df, w0, w1, today_ts, statuses, excl_dur, excl_co,
                                            now=pd.Timestamp(tzutil.now_ist()))
    _t = _lap("calculate", _t)
    # 21-Jul-2026 IST: reserved-slot booking-gap table, live dashboard only (not wired into main()/write_excel())
    rbg = ca.reserved_booking_gaps(inst, consumed_df, statuses)
    _t = _lap("reserved_booking_gaps", _t)
    data = dashboard.build_data_from_engine(g, open7, f"{w0.date()} to {w1.date()}",
                                            ", ".join(sorted(statuses)), n0, n1, blk7=blk7, rbg=rbg)
    _lap("build_data", _t)
...
def main():
    ...
    ap.add_argument("--status", default=DEFAULT_STATUSES, help="occupied statuses (default P,F,N; M & C excluded)")
    ...
    if a.step in ("fetch", "all"):
        ...
        # consumed window auto-follows the 7-day finder window so month-boundary spillover
        # (e.g. window ends 31-Jul but finder runs into Aug) still has consumed data to subtract
        consumed_end = max(w1, today + pd.Timedelta(days=6))
    ...
    # ---------- CALCULATE ----------
    slots_df = _apply_roles(slots_df)
    g, inst, open7, (n0, n1), blk7 = ca.calculate(slots_df, consumed_df, w0, w1, today, statuses, excl_dur, excl_co,
    ...
    ca.write_excel(g, inst, a.out, meta, open7=open7, blk7=blk7)
    print("saved", a.out)
